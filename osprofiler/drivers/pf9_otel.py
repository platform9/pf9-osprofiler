...
This driver addresses two critical issues with the upstream OTLP driver:
1. BatchSpanProcessor spawns threads that die on eventlet fork
2. HTTP exporter causes infinite recursion when trace_requests=true

Usage: connection_string = pf9_otel://otel-collector:4317
"""
...
class PF9OTel(base.Driver):
    """Platform9 OTLP driver using gRPC and SimpleSpanProcessor."""

    def __init__(self, connection_str, project=None, service=None, host=None,
                 conf=cfg.CONF, **kwargs):
        """Initialize the Platform9 OTLP driver.

        Uses gRPC exporter instead of HTTP to avoid infinite recursion
        when trace_requests=true. Uses SimpleSpanProcessor instead of
        BatchSpanProcessor for eventlet compatibility.
        """
        ...
        try:
            ...
            raise exc.CommandError(
                "To use pf9_otel driver, install: "
        ...
        # gRPC endpoint format: host:port (no scheme, no path)
    ...
    def create_span_tags(self, payload):
        """Create tags for an OpenTelemetry compatible span.

        :param payload: Payload from OSProfiler trace.
